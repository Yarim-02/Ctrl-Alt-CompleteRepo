...
void Level3::update()
{
	...
	{
		m_pLeaves[i]->getTransform()->position = m_pCamera->getTransform()->position + m_pLeaves[i]->getOffset();

		if (CollisionManager::PlatformCheck(m_pPlayer, m_pLeaves[i], m_pCamera) == true)
...
void Level3::start()
{
	...
	m_pPlayer->getTransform()->position = glm::vec2(800.0f, 600.0f);
	addChild(m_pPlayer);
...
void Level3::GUI_Function() const
{
